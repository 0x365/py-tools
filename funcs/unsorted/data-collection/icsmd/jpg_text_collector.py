...
def get_data(pdf, json_name):
    """
    Converts desired jpg info to json format
    """
...
pdf_files = []
pdf_folders = os.listdir(os.path.join(os.path.expanduser('~'), "local-data", "all-icsmd-data"))
for dir in pdf_folders:
    files = os.listdir(os.path.join(os.path.expanduser('~'), "local-data", "all-icsmd-data", dir))
    for fil in files:
...
for pdf in pdf_files:
    ...
    try:
        ...
        try:
            ...
            path = os.path.join(os.path.expanduser('~'), "local-data", "all-icsmd-data", pdf[0], pdf[1])
